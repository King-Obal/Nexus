...
// ── Tiny API helpers (via IPC, not direct fetch — CSP blocks localhost) ──────

function tGet(path)        { return window.forgeApi.get(path); }
function tPost(path, body) { return window.forgeApi.post(path, body); }
function tDel(path)        { return window.forgeApi.delete(path); }
function tSleep(ms)        { return new Promise(r => setTimeout(r, ms)); }

async function tGetState(id)                              { return tGet(`/api/game/${id}/state`); }
async function tRespond(id, body)                         { return tPost(`/api/game/${id}/respond`, body); }
async function tAddCard(id, card, zone = 'battlefield', player = 0) {
  return tPost(`/api/game/${id}/debug/add-card`, { card, zone, player });
}
async function tConcede(id) {
  try { await tPost(`/api/game/${id}/concede`, { player: 0 }); } catch {}
}

// ── Game navigation ───────────────────────────────────────────────────────────

class StopNav { constructor(state) { this.state = state; } }

async function tNavigate(id, handler, log, timeoutMs = 90_000) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    const state = await tGetState(id);
    if (state.gameOver) throw new Error(`Partie terminée prématurément (gagnant: ${state.winner})`);
    const dec = state.pendingDecision;
    if (!dec) { await tSleep(400); continue; }

    let resp;
    try {
      resp = await handler(state, dec);
    } catch (e) {
      if (e instanceof StopNav) return e.state || state;
      throw e;
    }

    if (resp == null) resp = tAutoHandle(state, dec, log);
    if (resp != null) await tRespond(id, resp);
    await tSleep(100);
  }
  const s = await tGetState(id);
  throw new Error(`Timeout — dernière décision: ${s.pendingDecision?.type ?? 'aucune'}, phase: ${s.phase}`);
}

function tAutoHandle(state, dec, log) {
  switch (dec.type) {
    case 'MULLIGAN':       return { keep: true };
    case 'CHOOSE_ACTION':  return { choice: 'pass' };
    case 'CONFIRM_TRIGGER':
    case 'CONFIRM_ACTION': return { choice: 'yes' };
    case 'CHOOSE_COLOR':   return { color: (dec.data?.colors ?? ['Green'])[0] };
    case 'CHOOSE_OPTION':  return { choice: (dec.data?.options ?? [''])[0] };
    case 'CHOOSE_NUMBER':  return { number: dec.data?.min ?? 0 };
    case 'CHOOSE_CARD':
      if (dec.data?.optional) return { cardIds: [] };
      return { cardIds: [(dec.data?.cards ?? [])[0]?.id].filter(Boolean) };
    default: return null;
  }
}

function tFindManaOpt(opts, cardName) {
  const forCard = opts.filter(o => o.card?.toLowerCase() === cardName.toLowerCase());
  return forCard.find(o => o.description?.toLowerCase().includes('add')) ?? forCard[0] ?? null;
}
function tFindCardOpt(opts, cardName) {
  return opts.find(o => o.card?.toLowerCase() === cardName.toLowerCase()) ?? null;
}

// ── Assertions ────────────────────────────────────────────────────────────────

function tAssertMana(state, pi, expected) {
  const pool = state.players[pi]?.manaPool ?? {};
  for (const [k, v] of Object.entries(expected)) {
    const actual = pool[k] ?? 0;
    if (actual < v) throw new Error(`Mana: attendu ${k}≥${v}, obtenu ${actual}. Pool: ${JSON.stringify(pool)}`);
  }
}
function tAssertManaExact(state, pi, expected) {
  const pool = state.players[pi]?.manaPool ?? {};
  const total = Object.values(pool).reduce((a, b) => a + b, 0);
  const expTotal = Object.values(expected).reduce((a, b) => a + b, 0);
  tAssertMana(state, pi, expected);
  if (total !== expTotal) throw new Error(`Trop de mana: attendu ${expTotal} total, obtenu ${total}. Pool: ${JSON.stringify(pool)}`);
}
function tAssertCardType(state, pi, cardName, typeFragment) {
  const bf = state.players[pi]?.battlefield ?? [];
  const card = bf.find(c => c.name === cardName);
  if (!card) throw new Error(`Carte "${cardName}" introuvable sur le battlefield`);
  if (!card.type?.includes(typeFragment))
    throw new Error(`"${cardName}" type="${card.type}" ne contient pas "${typeFragment}"`);
}
...
const TEST_SCENARIOS = [
  ...
  {
    id: 'city-of-traitors',
    name: 'City of Traitors — tap → {C}{C}',
    async run(id, log) {
      let setup = false, activated = false;
      await tNavigate(id, async (state, dec) => {
        if (dec.type === 'MULLIGAN' && !setup) {
          setup = true;
          await tAddCard(id, 'City of Traitors', 'battlefield', 0);
          log('City of Traitors ajoutée au champ');
          return { keep: true };
        }
        if (dec.type === 'CHOOSE_ACTION' && state.phase === 'MAIN1'
            && state.priorityPlayer === 'Player 1' && !activated) {
          const opt = tFindManaOpt(dec.data?.options ?? [], 'City of Traitors');
          if (opt) { activated = true; log('Activation de la capacité mana…'); return { choice: opt.id }; }
        }
        if (activated && dec.type === 'CHOOSE_ACTION') throw new StopNav(state);
        return null;
      }, log);
      if (!activated) throw new Error('Capacité mana de City of Traitors jamais trouvée dans les options');
      const fresh = await tGetState(id);
      tAssertMana(fresh, 0, { C: 2 });
      log(`✓ Mana pool: C=${fresh.players[0].manaPool.C}`);
    }
  },

  {
    id: 'wild-growth',
    name: 'Wild Growth — Forest enchantée → {G}{G}',
    async run(id, log) {
      let setup = false, casted = false, targeted = false, activated = false;
      await tNavigate(id, async (state, dec) => {
        if (dec.type === 'MULLIGAN' && !setup) {
          setup = true;
          await tAddCard(id, 'Forest', 'battlefield', 0);
          await tAddCard(id, 'Wild Growth', 'hand', 0);
          log('Forest + Wild Growth ajoutés');
          return { keep: true };
        }
        if (dec.type === 'CHOOSE_ACTION' && state.phase === 'MAIN1'
            && state.priorityPlayer === 'Player 1' && !casted) {
          const opt = tFindCardOpt(dec.data?.options ?? [], 'Wild Growth');
          if (opt) { casted = true; log('Cast Wild Growth…'); return { choice: opt.id }; }
        }
        if (dec.type === 'CHOOSE_CARD' && casted && !targeted) {
          targeted = true;
          const cards = dec.data?.cards ?? [];
          const forest = cards.find(c => c.name === 'Forest') ?? cards[0];
          if (!forest) throw new Error('Aucun land disponible pour cibler Wild Growth');
          log(`Cible: ${forest.name}`);
          return { cardIds: [forest.id] };
        }
        if (dec.type === 'CHOOSE_ACTION' && state.phase === 'MAIN1'
            && state.priorityPlayer === 'Player 1' && targeted && !activated) {
          const opt = tFindManaOpt(dec.data?.options ?? [], 'Forest');
          if (opt) { activated = true; log('Tap Forest pour mana…'); return { choice: opt.id }; }
        }
        if (activated && dec.type === 'CHOOSE_ACTION') throw new StopNav(state);
        return null;
      }, log);
      if (!activated) throw new Error('Jamais arrivé à taper la Forest après Wild Growth');
      const fresh = await tGetState(id);
      tAssertMana(fresh, 0, { G: 2 });
      log(`✓ Mana pool: G=${fresh.players[0].manaPool.G}`);
    }
  },

  {
    id: 'wilds-growth',
    name: 'Wilds Growth — Forest enchantée → {G}{G}',
    async run(id, log) {
      let setup = false, casted = false, targeted = false, activated = false;
      await tNavigate(id, async (state, dec) => {
        if (dec.type === 'MULLIGAN' && !setup) {
          setup = true;
          await tAddCard(id, 'Forest', 'battlefield', 0);
          await tAddCard(id, 'Wilds Growth', 'hand', 0);
          log('Forest + Wilds Growth ajoutés');
          return { keep: true };
        }
        if (dec.type === 'CHOOSE_ACTION' && state.phase === 'MAIN1'
            && state.priorityPlayer === 'Player 1' && !casted) {
          const opt = tFindCardOpt(dec.data?.options ?? [], 'Wilds Growth');
          if (opt) { casted = true; log('Cast Wilds Growth…'); return { choice: opt.id }; }
        }
        if (dec.type === 'CHOOSE_CARD' && casted && !targeted) {
          targeted = true;
          const cards = dec.data?.cards ?? [];
          const forest = cards.find(c => c.name === 'Forest') ?? cards[0];
          if (!forest) throw new Error('Aucun land pour cibler Wilds Growth');
          log(`Cible: ${forest.name}`);
          return { cardIds: [forest.id] };
        }
        if (dec.type === 'CHOOSE_ACTION' && state.phase === 'MAIN1'
      ...
      if (!activated) throw new Error('Jamais arrivé à taper la Forest après Wilds Growth');
      const fresh = await tGetState(id);
      tAssertMana(fresh, 0, { G: 2 });
      log(`✓ Mana pool: G=${fresh.players[0].manaPool.G}`);
    }
  ...
  {
    id: 'utopia-sprawl',
    name: 'Utopia Sprawl — choisir Rouge → Forest produit {G}{R}',
    async run(id, log) {
      let setup = false, casted = false, targeted = false, colored = false, activated = false;
      await tNavigate(id, async (state, dec) => {
        if (dec.type === 'MULLIGAN' && !setup) {
          setup = true;
          await tAddCard(id, 'Forest', 'battlefield', 0);
          await tAddCard(id, 'Utopia Sprawl', 'hand', 0);
          log('Forest + Utopia Sprawl ajoutés');
          return { keep: true };
        }
        if (dec.type === 'CHOOSE_ACTION' && state.phase === 'MAIN1'
            && state.priorityPlayer === 'Player 1' && !casted) {
          const opt = tFindCardOpt(dec.data?.options ?? [], 'Utopia Sprawl');
          if (opt) { casted = true; log('Cast Utopia Sprawl…'); return { choice: opt.id }; }
        }
        if (dec.type === 'CHOOSE_CARD' && casted && !targeted) {
          targeted = true;
          const cards = dec.data?.cards ?? [];
          const forest = cards.find(c => c.name === 'Forest') ?? cards[0];
          if (!forest) throw new Error('Aucun land pour cibler Utopia Sprawl');
          log(`Cible: ${forest.name}`);
          return { cardIds: [forest.id] };
        }
        if (dec.type === 'CHOOSE_COLOR' && !colored) {
          colored = true;
          log('Choix de couleur: Rouge');
          return { color: 'Red' };
        }
        if (dec.type === 'CHOOSE_ACTION' && state.phase === 'MAIN1'
            && state.priorityPlayer === 'Player 1' && (targeted || colored) && !activated) {
          const opt = tFindManaOpt(dec.data?.options ?? [], 'Forest');
          if (opt) { activated = true; log('Tap Forest pour mana…'); return { choice: opt.id }; }
        }
        if (activated && dec.type === 'CHOOSE_ACTION') throw new StopNav(state);
      ...
      if (!activated) throw new Error('Jamais arrivé à taper la Forest après Utopia Sprawl');
      const fresh = await tGetState(id);
      tAssertManaExact(fresh, 0, { G: 1, R: 1 });
      const pool = fresh.players[0].manaPool;
      log(`✓ Mana pool: G=${pool.G} R=${pool.R} (total=2, pas toutes les couleurs)`);
    }
  },

  {
    id: 'creature-land-zone',
    name: 'Creature-land — land earthbendé a le type "Creature"',
    async run(id, log) {
      let setup = false, activated = false, targeted = false;
      await tNavigate(id, async (state, dec) => {
        if (dec.type === 'MULLIGAN' && !setup) {
          setup = true;
          await tAddCard(id, 'Badgermole Cub', 'battlefield', 0);
          await tAddCard(id, 'Forest', 'battlefield', 0);
          log('Badgermole Cub + Forest ajoutés');
          return { keep: true };
        }
        if (dec.type === 'CHOOSE_ACTION' && state.phase === 'MAIN1'
            && state.priorityPlayer === 'Player 1' && !activated) {
          ...
          const opt = opts.find(o =>
            o.card === 'Badgermole Cub' && o.description &&
            (o.description.includes('becomes') || o.description.toLowerCase().includes('creature')
             || o.description.toLowerCase().includes('earthbend') || o.description.includes('Forest'))
          );
          if (opt) { activated = true; log('Activation earthbend du Badgermole Cub…'); return { choice: opt.id }; }
        }
        if (dec.type === 'CHOOSE_CARD' && activated && !targeted) {
          targeted = true;
          const cards = dec.data?.cards ?? [];
          const forest = cards.find(c => c.name === 'Forest') ?? cards[0];
          if (!forest) throw new Error('Aucun land pour earthbend');
          log(`Cible: ${forest.name}`);
          return { cardIds: [forest.id] };
        }
        if (activated && targeted && dec.type === 'CHOOSE_ACTION') throw new StopNav(state);
        return null;
      }, log);
      if (!activated) throw new Error('Capacité earthbend du Badgermole Cub introuvable (description inattendue?)');
      const fresh = await tGetState(id);
      tAssertCardType(fresh, 0, 'Forest', 'Creature');
      const card = fresh.players[0].battlefield.find(c => c.name === 'Forest');
      log(`✓ Type: "${card.type}"`);
    }
  },
  ...
  for (const s of TEST_SCENARIOS) {
    ...
    card.innerHTML = `
      <div class="test-header">
        <div class="test-badge" id="tbadge-${s.id}">●</div>
        <div class="test-name">${esc(s.name)}</div>
        <div class="test-time" id="ttime-${s.id}"></div>
        <div class="test-toggle">▶</div>
      </div>
      <div class="test-log" id="tlog-${s.id}"></div>`;
...
function testsSetRunning(id) {
  const badge = document.getElementById('tbadge-' + id);
  if (badge) { badge.className = 'test-badge running'; badge.textContent = '●'; }
  const card = document.getElementById('tcard-' + id);
  if (card) card.classList.add('expanded');
}

function testsSetResult(id, passed, elapsed, err) {
  const card  = document.getElementById('tcard-' + id);
  const badge = document.getElementById('tbadge-' + id);
  const timeEl = document.getElementById('ttime-' + id);
  if (!card) return;
  if (passed) {
    ...
    badge.textContent = '✗';
    if (err) testsAddLog(id, '✗ ' + err, 'err');
  }
  if (elapsed != null) timeEl.textContent = (elapsed / 1000).toFixed(1) + 's';
}

function testsAddLog(id, text, cls = '') {
  const logEl = document.getElementById('tlog-' + id);
  if (!logEl) return;
  const line = document.createElement('div');
  line.className = 'test-log-line' + (cls ? ' ' + cls : '');
...
async function testsRun() {
  ...
  const summBar = document.getElementById('tests-summary-bar');
  const passEl  = document.getElementById('tests-count-pass');
  const failEl  = document.getElementById('tests-count-fail');

  btn.disabled = true;
  spinner.style.display = '';
  summBar.style.display = 'none';

  for (const s of TEST_SCENARIOS) testsResetCard(s.id);

  // Ensure test deck exists
  try {
    await tPost('/api/decks/import', {
      name: TEST_DECK_NAME,
      format: 'Constructed',
      mainboard: [{ name: 'Forest', qty: 60 }]
    });
  } catch (e) {
    spinner.textContent = 'Erreur: impossible de créer le deck test (' + e.message + ')';
    btn.disabled = false;
    testsRunning = false;
    return;
  }
  ...
  for (const scenario of TEST_SCENARIOS) {
    testsSetRunning(scenario.id);
    const log = (msg, cls = '') => testsAddLog(scenario.id, msg, cls);
    const t0 = Date.now();
    let gameId = null;
    try {
      const session = await tPost('/api/game/start', {
        deck1: TEST_DECK_NAME, deck2: TEST_DECK_NAME,
        format: 'Constructed', debug: true, goFirstPlayerIndex: 0
      });
      gameId = session.id;
      log('Partie démarrée: ' + gameId, 'info');
      await scenario.run(gameId, log);
      testsSetResult(scenario.id, true, Date.now() - t0);
      nPass++;
    } catch (e) {
      testsSetResult(scenario.id, false, Date.now() - t0, e.message);
      nFail++;
    } finally {
      if (gameId) await tConcede(gameId);
    }
  }

  // Cleanup deck
  try { await tDel('/api/decks?name=' + encodeURIComponent(TEST_DECK_NAME) + '&format=constructed'); } catch {}

  passEl.textContent = nPass + ' ✓';
  failEl.textContent = nFail + ' ✗';
...
testsInitCards();
document.getElementById('btn-run-tests').addEventListener('click', testsRun);
